...
/**
 * Build BrowserContext options, honoring A11Y_AUTH_STATE_FILE env var if set.
 *
 * If the env var points to a Playwright storageState JSON file (generated by
 * `await context.storageState({ path })` after login), we load it so every
 * scan runs as the authenticated user. This is how authenticated routes
 * (admin settings, dashboards, anything behind a login) get scanned without
 * the headless browser hitting a login wall.
 *
 * If the file is missing or unreadable, we fall back to unauthenticated and
 * let the tool surface whatever the public view is — no silent failures.
 */
async function buildContextOptions(extra = {}) {
  const statePath = process.env.A11Y_AUTH_STATE_FILE;
  const options = { ...extra };
  if (statePath) {
    ...
    try {
      ...
      // State file missing or unreadable; continue unauthenticated.
    }
  }
  return options;
}
  ...
  server.registerTool(
    ...
    async ({ url, tags, include, exclude }) => {
      ...
      try {
        ...
        try {
          // @axe-core/playwright >=4.11 requires an explicit BrowserContext —
          // browser.newPage() creates one implicitly but the stricter runtime
          // check refuses it with "Please use browser.newContext()".
    ...
    async ({ url, root }) => {
      ...
      try {
        ...
        try {
          ...
          await page.goto(safeUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
          await page.waitForLoadState("networkidle", { timeout: 15000 }).catch(() => {});
          // ariaSnapshot lives on Locator, not Page — wrap the page when no root
          // selector is supplied. Without this wrap Playwright throws
          // "page.ariaSnapshot is not a function".
        ...
        try {
          ...
          for (const width of widths) {
            const context = await browser.newContext(
              await buildContextOptions({ viewport: { width, height: 800 } })
            );
            const page = await context.newPage();
            await page.goto(safeUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
          await page.waitForLoadState("networkidle", { timeout: 15000 }).catch(() => {});
